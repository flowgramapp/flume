...
describe('<NodeEditor/>', () => {
  ...
  test('Component renders', async () => {
    render(<NodeEditor nodes={exampleNodes} nodeTypes={nodeTypes} portTypes={portTypes} />);
  });
});
  ...
  test('Can input numbers', async () => {
    const {container, getByText} = render(
      <div style={{width: 900, height: 700}}>
        <NodeEditor nodes={exampleNodes} nodeTypes={nodeTypes} portTypes={portTypes} debug />
      </div>
    );
