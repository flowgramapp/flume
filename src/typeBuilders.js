    ...
    if (!this.portTypes[type]) {
      ...
      if (!skipDynamicNodesCheck) {
        const dynamicNodes = Object.values(this.nodeTypes).filter(
          node => typeof node.inputs === 'function' || typeof node.outputs === 'function'
        );
      ...
      const affectedNodes = Object.values(this.nodeTypes).filter(
        node =>
          (Array.isArray(node.inputs) && node.inputs.find(p => p.type === type)) ||
          (Array.isArray(node.outputs) && node.outputs.find(p => p.type === type))
      );
